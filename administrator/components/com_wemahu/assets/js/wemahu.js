jQuery(document).ready(function($) {
	...
	$.handleAddToList = function() {
		$('.wmAjaxAddToList').click(function(event) {
			event.preventDefault();
			var task = 'dashboard.' + $(this).data('task');
			var reportId = $(this).closest('form').find('input[name="report_id"]').val();
			var ajaxRand = new Date().getTime();
			$.ajaxQueue({
				type: "POST",
				url: 'index.php?option=com_wemahu&format=json',
				dataType: 'json',
				data: 'task=' + task + '&reportId=' + reportId + '&rv=' + ajaxRand,
			...
			case 'report_success':
				...
				$('.wmReportItem').click(function(e) {
					...
					$.ajaxQueue({
						...
						url: 'index.php?option=com_wemahu&format=json',
						dataType: 'json',
						data: 'task=dashboard.getReportItemModal&reportId=' + reportId + '&rv=' + ajaxRand,
						success: function(jsonResponse) {
							switch(jsonResponse.type) {
								case 'success':
	...
	$.setProgress = function(percent) {
		...
		$('#wmProgress .bar').css('width', percent + '%');
	}
